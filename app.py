# ============================================
# VetControl Pro - ULTIMA VERSION 3.0
# ============================================
...
def init_sale_selector():
    if "sale_selected_product_label" not in st.session_state:
        st.session_state["sale_selected_product_label"] = ""
...
def fetch_clientes():
    try:
        return (
            ...
            .select("id, nombre, telefono, puntos_lealtad")
            .order("nombre")
            .execute()
            .data
            or []
...
def fetch_ventas(start_date=None, end_date=None):
    query = (
        ...
        .select("id, fecha, metodo_pago, total, observacion, estado, cliente_id, clientes(nombre)")
...
def compute_product_metrics(producto_id, lotes_data, precio_referencia=None):
    lotes_prod = [
        l
        for l in lotes_data
        if l.get("producto_id") == producto_id and safe_int(l.get("cantidad_actual")) > 0
    ]
    stock_total = sum(safe_int(l.get("cantidad_actual")) for l in lotes_prod)
    costo_total = sum(
        safe_int(l.get("cantidad_actual")) * safe_float(l.get("costo_unidad")) for l in lotes_prod
    )
...
def allocate_stock_for_sale(producto_id, cantidad, lotes):
    lotes_data = [
        l for l in lotes if l.get("producto_id") == producto_id and safe_int(l.get("cantidad_actual")) > 0
    ]
    lotes_data = sorted(
        lotes_data,
        key=lambda x: (str(x.get("fecha_vencimiento") or "9999-12-31"), str(x.get("fecha_ingreso") or "")),
    )
        ...
        allocations.append(
            {
                "lote_id": lote["id"],
                "cantidad": quitar,
                "costo": safe_float(lote.get("costo_unidad")),
    ...
    for enc in encodings:
        for delim in delimiters:
            try:
                ...
                if {"categoria", "producto", "cantidad"}.issubset(set(df.columns)):
                    return df
            except Exception as e:
                last_error = e
                continue
    raise last_error if last_error else ValueError("No se pudo leer el CSV.")
...
if check_password():
    ...
    # ENTRADAS
        ...
        with tab2:
            if lotes:
                labels = {
                    f"{(l.get('productos') or {}).get('nombre', '-')} | Lote: {l.get('lote') or '-'} | {format_dt(l.get('fecha_ingreso'))}": l
                    for l in lotes
                }
                ...
                rows.append(
                    {
                        ...
                        "Vencimiento": format_dt(item.get("fecha_vencimiento"), False),
    ...
    # VENTAS
        ...
        if not productos:
            ...
            with left:
                ...
                if not clientes:
                    ...
                    selected_label = st.selectbox(
                        "Producto",
                        options=[""] + list(prods.keys()),
                        format_func=lambda x: "Selecciona un producto..." if x == "" else x,
                        key="sale_selected_product_label",
                    )

                    if selected_label == "":
                        st.info("Selecciona un producto para cargar stock, unidad, costo y precio.")
                    else:
                        p_data = prods[selected_label]
            ...
            with right:
                ...
                if cart_items:
                    ...
                    st.dataframe(
                        show_df[["Producto", "Unidad", "Cantidad", "Precio", "Subtotal", "Margen", "Utilidad"]],
        ...
        if ventas:
            ...
            if ventas_activas:
                venta_opts = {
                    f"{format_dt(v.get('fecha'))} | {(v.get('clientes') or {}).get('nombre', '-')} | {format_bs(v.get('total'))}": v["id"]
    ...
    # REPORTES
                ...
                rows.append(
                    {
                        ...
                        "Cantidad": cantidad,
                        "Precio unitario": format_bs(precio),
                        "Ingreso": format_bs(cantidad * precio),
                        "Costo": format_bs(cantidad * costo),
                        "Utilidad bruta": format_bs((cantidad * precio) - (cantidad * costo)),
        ...
        with tab2:
            ...
            if not cli_df.empty:
                resumen = (
                    cli_df.groupby("Cliente", as_index=False)
                    .agg(
                        Compras=("Total", "count"),
                        Total=("Total", "sum"),
        ...
        with tab4:
            ...
            if not stock_df.empty:
                st.dataframe(
                    stock_df[["Producto", "Unidad", "Stock actual", "Stock mínimo", "Estado"]],
                    use_container_width=True,
                    hide_index=True,
                )
    ...
    # IMPORTACION MASIVA
        ...
        if archivo is not None:
            try:
                df = read_uploaded_sheet(archivo)
                df.columns = [str(c).strip().lower() for c in df.columns]
    ...
    st.markdown("<div class='footer-version'>VetControl Pro - ULTIMA VERSION 3.0</div>", unsafe_allow_html=True)
